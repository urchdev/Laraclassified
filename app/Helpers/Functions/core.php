...
{
	return (getHost() == getHost(env('APP_URL')) && file_exists(storage_path('framework/plugins/domainmapping')));
}
...
function genEmailContactBtn($post = null, $btnBlock = false, $iconOnly = false)
{
	...
	if ($iconOnly) {
		...
		$out .= '<i class="icon-mail-2" data-bs-toggle="tooltip" title="' . t('Send a message') . '"></i>';
		$out .= '</a>';
	} else {
		if ($btnBlock) {

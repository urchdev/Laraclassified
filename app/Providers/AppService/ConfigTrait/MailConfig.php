...
trait MailConfig
{
	...
	private function updateMailConfig($settings = 'settings')
	{
		...
		if (config('mail.default') == 'sendmail') {
			...
			config()->set('mail.mailers.smtp.host', env('MAIL_HOST', config('settings.mail.sendmail_host', $mailHost)));
			config()->set('mail.mailers.smtp.port', env('MAIL_PORT', config('settings.mail.sendmail_port', $mailPort)));
			config()->set('mail.mailers.smtp.encryption', env('MAIL_ENCRYPTION', config('settings.mail.sendmail_encryption', $mailEncryption)));
			config()->set('mail.mailers.smtp.username', env('MAIL_USERNAME', config('settings.mail.sendmail_username', $mailUsername)));
			config()->set('mail.mailers.smtp.password', env('MAIL_PASSWORD', config('settings.mail.sendmail_password', $mailPassword)));
		...
		if (config('mail.default') == 'mailgun') {
			...
			config()->set('services.mailgun.endpoint', env('MAILGUN_ENDPOINT', config('settings.mail.mailgun_endpoint', 'api.eu.mailgun.net')));
		...
		if (config('mail.default') == 'mandrill') {
			...
			config()->set('mail.mailers.smtp.host', env('MAIL_HOST', config('settings.mail.mandrill_host', $mailHost)));
			config()->set('mail.mailers.smtp.port', env('MAIL_PORT', config('settings.mail.mandrill_port', $mailPort)));
			config()->set('mail.mailers.smtp.encryption', env('MAIL_ENCRYPTION', config('settings.mail.mandrill_encryption', $mailEncryption)));
			config()->set('mail.mailers.smtp.username', env('MAIL_USERNAME', config('settings.mail.mandrill_username', $mailUsername)));
			config()->set('mail.mailers.smtp.password', env('MAIL_PASSWORD', config('settings.mail.mandrill_password', $mailPassword)));
			config()->set('mail.from.address', env('MAIL_FROM_ADDRESS', config('settings.mail.mandrill_email_sender', $mailSender)));
		}
